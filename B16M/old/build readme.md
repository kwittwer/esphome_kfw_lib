PowerShell Script für automatisches Versioning und Build 🚀
Hier ist ein komplettes Script, das:

Die Version in b16m.yaml aktualisiert (Format: YYYY.MM.DD.C)
ESPHome kompiliert
Bei Erfolg das .bin File nach Builds\ kopiert

Das Script
Erstelle eine neue Datei build-and-version.ps1:
...
$binSourcePath = Join-Path $projectPath ".esphome\build\b16m\b16m.bin"
...
if ($yamlContent -match 'version:\s*["\']?[\d\.]+["\']?') {
    ...
    $yamlContent = $yamlContent -replace '(version:\s*)["\']?[\d\.]+["\']?', "`${1}""$newVersion"""
} else {
    # Version existiert nicht, füge sie nach "esphome:" ein
    if ($yamlContent -match '(esphome:\s*\n)') {
        $yamlContent = $yamlContent -replace '(esphome:\s*\n)', "`${1}  version: ""$newVersion""`n"
    } else {
...
if ($process.ExitCode -eq 0) {
    ...
    Write-ColorOutput Green "=========================================="
    Write-ColorOutput Yellow "Dauer: $([math]::Round($compileDuration, 2)) Sekunden"
    Write-Host ""
    ...
    if (Test-Path $binSourcePath) {
        ...
        Write-ColorOutput Yellow "  Größe: $fileSizeMB MB"
        Write-Host ""
...
Build Duration: $([math]::Round($compileDuration, 2)) seconds
File Size:      $fileSizeMB MB
Source:         $yamlFile
Binary:         $binFileName

ESPHome Version: $(esphome version 2>&amp;1 | Select-String "Version:")
"@
        ...
        Get-ChildItem -Path $buildsFolder -Filter "b16m_v*.bin" | 
            ...
            ForEach-Object {
                ...
                Write-Host "  $($_.Name.PadRight(30)) $date  ($size MB)"
            }
    ...
    Write-ColorOutput Yellow "Setze Version zurück auf: $oldVersion"
    if ($oldVersion -ne "keine") {
        $yamlContent = Get-Content $yamlFile -Raw
        $yamlContent = $yamlContent -replace '(version:\s*)["\']?[\d\.]+["\']?', "`${1}""$oldVersion"""
        $yamlContent | Set-Content $yamlFile -NoNewline
        Write-ColorOutput Green "✓ Version zurückgesetzt"
    }
...
Verwendung
1. Einfacher Build (Counter wird automatisch erhöht):
.\build-and-version.ps1

2. Build ohne Counter-Erhöhung:
.\build-and-version.ps1 -IncrementCounter:$false

3. Counter manuell zurücksetzen:
.\build-and-version.ps1 -ResetCounter


Was das Script macht
Schritt 1: Versioning

Liest aktuelle Version aus b16m.yaml
Erstellt neue Version: YYYY.MM.DD.C
Bei neuem Datum: Counter = 1
Sonst: Counter + 1
Aktualisiert version: in der YAML

Schritt 2: Kompilierung

Führt esphome compile b16m.yaml aus
Zeigt Fortschritt an
Misst Kompilierungszeit

Schritt 3: Bei Erfolg

Kopiert .bin nach Builds\b16m_vYYYY.MM.DD.C.bin
Erstellt b16m_latest.bin (immer aktuellste Version)
Erstellt Build-Info .txt Datei
Zeigt die letzten 5 Builds an

Schritt 4: Bei Fehler

Setzt Version in YAML zurück
Zeigt Fehlermeldung


Beispiel-Output
==========================================
  ESPHome Build &amp; Version Manager
==========================================

Aktuelle Version: 2025.12.01.3
Counter erhöht: 3 → 4
Neue Version: 2025.12.02.1

✓ Version in b16m.yaml aktualisiert
...
==========================================
Dauer: 45.23 Sekunden

✓ Binary kopiert nach:
  C:\Users\...\Builds\b16m_v2025.12.02.1.bin
  Größe: 1.45 MB

✓ Latest-Version erstellt: b16m_latest.bin
✓ Build-Info erstellt: b16m_v2025.12.02.1.txt

==========================================
  Build abgeschlossen!
==========================================
Version:  2025.12.02.1
Datei:    b16m_v2025.12.02.1.bin
Ordner:   C:\Users\...\Builds

Letzte Builds:
  b16m_v2025.12.02.1.bin         2025-12-02 09:30  (1.45 MB)
  b16m_v2025.12.01.3.bin         2025-12.01 18:22  (1.44 MB)
  b16m_v2025.12.01.2.bin         2025-12-01 15:10  (1.44 MB)


Bonus: Alias erstellen
...
# Füge hinzu:
function Build-B16M {
    Set-Location "C:\Users\z004s49s\Documents\B16M_Homecontroller\B16M_Homecontroller"
    .\build-and-version.ps1 @args
}

Set-Alias -Name build -Value Build-B16M

Dann kannst du einfach schreiben:
build


Struktur nach mehreren Builds
B16M_Homecontroller/
├── b16m.yaml
├── build-and-version.ps1
└── Builds/
    ├── b16m_latest.bin
    ├── b16m_v2025.12.02.1.bin
    ├── b16m_v2025.12.02.1.txt
    ├── b16m_v2025.12.01.3.bin
    ├── b16m_v2025.12.01.3.txt
    └── ...
